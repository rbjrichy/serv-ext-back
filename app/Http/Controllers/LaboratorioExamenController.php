...
use App\Http\Requests\LaboratorioExamen\StoreLaboratorioExamenRequest;
...
class LaboratorioExamenController extends Controller
{
    public function index(Request $request)
    {
        try {
            $data = LaboratorioExamen::buscar($request);
            return response()->json([
                'error' => false,
                'data' => $data,
                'mensaje' => 'Lista de examenes laboratorio',
                'status' => 200,
            ], 200);
        } catch (\Exception $e) {
    ...
    {
        try {
            $data = LaboratorioExamen::where('id', $id)->first();
            if (!$data) {
                throw new HttpException(404, 'El examen no existe');
            }
            return response()->json([
                'error' => false,
                'data' => $data,
    ...
    public function store(StoreLaboratorioExamenRequest $request)
    {
        try {
            DB::beginTransaction();
            ...
            $categoria = LaboratorioExamen::where('nombre', $validado['nombre'])
                ->first();
            if ($categoria) {
            ...
            $data = LaboratorioExamen::create($validado);
    ...
    public function update($id, StoreLaboratorioExamenRequest $request)
    {
        try {
            DB::beginTransaction();
            $user = Auth::getPayload()->get('user');
            $validado = $request->validated();
            $categoria = LaboratorioExamen::find($id);
            if (!$categoria) {
                throw new HttpException(404, 'No se encontro la categoria');
            }

            $categoria_repetida = LaboratorioExamen::where('nombre', $validado['nombre'])
                ->where('id', '<>', $id)
                ->first();
    ...
    public function baja($id, StoreObservacionRequest $request)
    {
        try {
            $user = Auth::getPayload()->get('user');
            $data = LaboratorioExamen::find($id);
            $validado = $request->validated();
    ...
    public function reincorporar($id, StoreObservacionRequest $request)
    {
        try {
            ...
            if (!$data) {
                throw new HttpException(404, 'Examen no encontrado');
            }
    ...
    public function maximo_orden_lista(Request $request)
    {
        try {
            $categoria_id = $request->input('categoria_id');
            $subcategoria_id = $request->input('subcategoria_id');
            $data = LaboratorioExamen::where('laboratorio_categoria_id', $categoria_id)
                ->where('laboratorio_subcategoria_id', $subcategoria_id)
